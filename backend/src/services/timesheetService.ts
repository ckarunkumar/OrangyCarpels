...
export class TimesheetService {
  ...
  static async getWeeklySheet(weekStart: string): Promise<WeeklyTimesheet> {
    let sheet = await prisma.timesheet.findUnique({
      where: { weekStart },
      include: { rows: true },
    });

    if (!sheet) {
      sheet = await prisma.timesheet.create({
        data: {
          weekStart,
          status: 'Draft',
        },
    ...
    return {
      ...
      status: sheet.status as WeeklyTimesheet['status'],
  ...
  static async saveDraft(
    ...
    role: string,
    rows: TimesheetRow[]
  ): Promise<{ success: boolean; data?: WeeklyTimesheet; error?: string }> {
    ...
    try {
      const dbSheet = await prisma.timesheet.findUnique({
        where: { weekStart },
      });
      if (!dbSheet) throw new Error('Timesheet period not initialized.');
   ...
   */
  static async approveOrRejectSheet(
    weekStart: string,
    role: string,
    ...
    let nextStatus: WeeklyTimesheet['status'] = 'Draft';
    if (action === 'approve') {
      if (sheet.status === 'Approved' && role !== 'Super Admin') {
