...
const EMPTY_FORM: FormState = {
  employeeId: '', fullName: '', dob: '', designation: '', department: '',
  email: '', personalEmail: '', phone: '', secondaryPhone: '',
  permanentAddress: '', guardianName: '', motherName: '', bloodGroup: '',
  linkedInUrl: '', aadhaarNumber: '', panNumber: '', status: 'Active', role: 'Employee',
};
...
export default function EmployeeDrawer({ open, mode, employee, onClose, onSaved }: EmployeeDrawerProps) {
  ...
  const inputCls = (hasErr?: boolean) =>
    `w-full px-3 py-2 border rounded text-[12.5px] text-studio-text bg-white transition-colors focus:outline-none ${
  ...
  return (
    <>
      ...
      <div className={`fixed top-0 right-0 z-50 h-full w-full max-w-4xl bg-white shadow-xl flex flex-col transition-transform duration-250 ease-out ${open ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex items-center justify-between px-7 py-4 border-b border-studio-border shrink-0">
          <div>
            <h3 className="text-[16px] font-semibold text-studio-text tracking-tight">{mode === 'edit' ? 'Edit Employee' : 'New Employee'}</h3>
            <p className="text-[11px] text-studio-muted mt-0.5">{mode === 'edit' ? `Editing: ${employee?.fullName} (${employee?.employeeId || employee?.id})` : 'Enter employee details below'}</p>
          </div>
        ...
        <form id="employee-form" onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-7 py-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-x-6 gap-y-4">
            ...
            <div>
              ...
              <input ref={firstInputRef} type="text" placeholder="EMP-001" value={form.employeeId} onChange={set('employeeId')} className={`${inputCls()} font-mono`} />
            </div>
            <div>
              <label className={labelCls}>Full Name *</label>
              <input type="text" placeholder="e.g. Maya Lin" value={form.fullName} onChange={set('fullName')} className={inputCls(!!errors.fullName)} />
            ...
            <div>
              <label className={labelCls}>Second Phone</label>
            ...
            <div>
              ...
              <input type="text" placeholder="House no, Street, City, PIN" value={form.permanentAddress} onChange={set('permanentAddress')} className={inputCls()} />
            </div>
            <div>
        ...
        <div className="shrink-0 px-7 py-4 border-t border-studio-border flex items-center justify-end gap-3">
          <button type="button" onClick={onClose} className="px-4 py-2 text-[12px] font-medium text-studio-muted hover:text-studio-text transition-colors">
            Cancel
          </button>
          <button type="submit" form="employee-form" disabled={saving} className="px-4 py-2 text-[12px] font-semibold text-white bg-brand-orange rounded hover:bg-opacity-95 transition-colors disabled:opacity-50">
