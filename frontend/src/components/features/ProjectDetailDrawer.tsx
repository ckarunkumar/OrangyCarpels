import { X, FolderKanban, Building2, Pencil } from 'lucide-react';
...
export default function ProjectDetailDrawer({
  ...
  const consumption = Math.round((project.loggedHours / (project.budgetHours || 1)) * 100);
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div className="bg-white border border-studio-border rounded-lg p-4 space-y-2.5">
            <div className="flex justify-between items-center text-[12px]">
              <span className="text-studio-muted font-medium">Budget Consumption</span>
            ...
            <div className="w-full h-2 bg-studio-bg rounded-full overflow-hidden border border-studio-border">
              <div
                className={`h-full transition-all ${
                  isExceeded ? 'bg-amber-500' : 'bg-brand-blue'
                }`}
          ...
          <div className="grid grid-cols-2 gap-3 bg-studio-sidebar border border-studio-border rounded p-3 text-[12px]">
            <div>
              ...
              <span className="font-semibold text-studio-text mt-0.5 block truncate">{project.billingType}</span>
            </div>
            <div>
